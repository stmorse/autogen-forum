...
(chs-slim branch note:)
Code-cleanup throughout, began late Fall 2024
"""
...
from utilities import logging_utils     # just need send_log_message
from utilities import event_utils

from utilities.utils import \
    get_activity_name, end_event, get_llm_response, generate_persona_seed, get_topic_random, \
...
from utilities.faiss_utils import \
    create_faiss_index, get_faiss_manager, add_vectors, delete_faiss_index, search_index, \
    get_vector_ids
...
#NOTE: This may cause deadlocks. In particular, this has to do with the forking 
# of the process in ask_question (among others) wherein the process of returning 
# the response happens first, and then the memory is stored into the FAISS memory database
import os
...
class CompHuSimAgent:
    def __init__(self, config=None, logger = None, connection_pool=None, this_uuid='', do_create=True):
        ...
        self.faiss_manager = get_faiss_manager(
            a=(self.faiss_host, int(self.faiss_port)), 
            key=b'faiss'
        )
        
        self._log(f"INFO: Comp-HuSim Agent Starting Up at {datetime.now()} " + \
                  f"on {self.logging_host}:{self.logging_port}")
        
        # Memory span for agent
        # how many memories to recall when "remembering" 
        # from FAISS DB (ie: k nearest neighbors)
        self.memory_span = 10  

        # Set up the current activity that the agent is engaged in.
        self.nullify_current_event()

        # Set up the current task list that the agent maintains.
        self.task_list = None
        ...
        if not self.connection_pool:
            ...
            self._log(f"ERROR: FAILED STARTUP: Error connecting to database. SHUTDOWN.")
            exit()

        # TODO: adapt for internal server
        ...
        if this_uuid:
            ...
            try:
                ...
                self._log(message=f"ERROR: FAILED TO LOAD AGENT: {e}")
                self.uuid = ''
        elif do_create:
            self.uuid = ''
            self.create()  # TODO: defaults=False?
            self.load_agent_details()
        else:
    ...
    def _log(self, message):
        ...
        logging_utils.send_log_message(
            message=message, 
    ...
    def update_db_column(self, column_name, column_value):
        ...
        try:
            ...
            # Log changes
            self._log(f"INFO: UPDATED AGENT DB. UUID: {self.uuid}")
    ...
    def load_agent_details(self):
        ...
        self._log(f"INFO: ====STARTING AGENT LOADING====")
        ...
        self._log(f"INFO: complete agent details load. UUID: {self.uuid}. Elapsed Time: {elapsed_time:.5g}")
        ...
        self._log(f"INFO: complete load agent memories. Elapsed Time: {elapsed_time2:.5g}")
    ...
    def get_current_event(self):
        #Return the agent's current_event dict
        return self.current_event

    def get_last_event(self):
        '''
        This function returns the last event that the agent has stored in its agent_events table.

        Args:

        Returns:
            returns the agent's last_event
        ...
        # Obsolete as of 20241203 However, this may be useful in future.
    ...
    def choose_next_event(self, activities_list=None, debug=False):
        # WARNING: The list of possible activities needs to be a SUBSET from daily_activities table.
        # Otherwise there will be downstream failures
        ...
        if self.current_event['activity_name'] == 'NO ACTIVITY':
        ...
        # Get last activity information, based on the agents's last event
        ...
        if last_event:
            last_activity_name = last_event['activity_name']
            last_activity_intention = "For this reason: \"" + last_event['intention'] + "\""
        else:
            last_activity_name = "NO ACTIVITY"
    ...
    def start_event(self, pick_random_daily_activity=False, activity_uuid=None, activity_name=None,
                    do_find_activity=False,
                    activity_description=None, create_daily_activity=False, generate_intention=False, intention=None,
                    context_info=None, debug=False):
        """
        ...
        # End current event if one already exists for agent. IE: we're moving onto the next event.
        ...
        if activity_uuid:
            ...
            if activity_name is None and activity_uuid is None and activity_description is None and pick_random_daily_activity is False:
                self._log(
                    "ERROR: Can't start activity without " + \
                    "activity_name, activity_uuid, activity_description OR pick_random"
                )
                return None
            ...
            if pick_random_daily_activity:
                ...
                activity_uuid = find_activity(cursor=cursor, activity_description=activity_description,
                                              config=self.config,
                                              create_activity=create_daily_activity)
                activity_name = get_activity_name(cursor, activity_uuid=activity_uuid)

            else:
                self._log(f"ERROR: Didn't have enough information to start Agent Event.")

        # Get the activity name:
        if activity_name is None:
            self._log(f"ERROR: No Event Activity Name Specified. FAIL.")
            return None

        if debug:
            print(f"Starting new activity: {activity_uuid}: {activity_name} ")
        ...
        current_event = {'agent_event_uuid': new_event_uuid,
        ...
        if intention:
            current_event['intention'] = intention

        if context_info:
            current_event['context_info'] = context_info
    ...
    def set_current_event(self, agent_event_uuid=None):
        ...
        if agent_event_uuid is None:
            ...
            self._log("ERROR: NO Agent Event UUID Provided.")
        ...
        if result:
            ...
            self._log("ERROR: No Agent Event found from provided UUID.")

        cursor.close()
        self.connection_pool.putconn(conn)

    def generate_event_intention(self, activity_uuid=None, agent_event_uuid=None, model='gpt-4o'):
        """
    ...
    def faiss_remember(self, search_string, nearest_neighbors=None, print_time=False, llm_check_memories=False, debug=False):
        ...
        if nearest_neighbors is None:
            ...
            self._log(f"Using default K Nearest Neighbors Setting: {nearest_neighbors}")
        ...
        self._log(f"Trying to search memory index for: \"{search_string}\", with K nearest neighbors: {nearest_neighbors}")
        ...
        self._log(f"INFO: FAISS-based CompHuSimAgent.remember() Elapsed Time: {elapsed_time:.5g} sec")
        ...
        if llm_check_memories and len(original_relevant_memories)>1:
            ...
            #Log Elapsed Time
            self._log(f"LLM Check Memories: Elapsed Time: {elapsed_time:.5g} sec")
    ...
    def get_memories_dict(self, memory_vector_ids=None, event_context_uuid=None, debug=False):
        ...
        self._log(f"Loading {len(rows)} memories for agent.")
        for row in rows:
    ...
    def sync_memory_vector_db(self,dimension=None,debug=False,recreate=False):
        ...
        #Recreate if desired...
        if recreate:
            self._log(f"Attempting to recreate VectorDB Index from Postgres. Deleting & Creating fresh...")
        ...
        if postgres_vector_db_id_list is None:
            ...
            self._log(f"INFO: Postgres DB is EMPTY. Stop execution here.")
            return
        ...
        if are_lists_equal(postgres_vector_db_id_list,faiss_vector_db_id_list):
            ...
            self._log(f"INFO: Vector DB Lists are EQUAL")
        else:
            memories_to_add_to_faiss = are_lists_equal(postgres_vector_db_id_list,faiss_vector_db_id_list,type='diff')
            self._log(f"INFO: Vector DB Lists are NOT EQUAL. Need to add {len(memories_to_add_to_faiss)} memories to FAISS index.")
            ...
            try:
                ...
                self._log(f"ERROR: FAILURE adding to FAISS index: {e}")
                raise
    ...
    def load_memory_vector_db(self, memories_dict=None, debug = False):
        ...
        if gpu_check_avail_faiss():
            ...
            self._log("INFO: Using CUDA for FAISS DB INDEX")
        else:
            # Initialize FAISS index for CPU
            if debug: print("Using FAISS CPU for FAISS DB Index")
            self._log("INFO: Using CPU for FAISS DB INDEX")
    ...
    def store_memory(self, summary, precise_memory = None, activity_uuid=None, activity_name=None, model='gpt-4o-mini', debug=False):
        """
        ...
        llm_prompt = f"""Imagine that YOU are the PERSON described by the PERSON TRAITS below. You have just now completed this ACTIVITY: {activity_name}.
                ...
                Do NOT include any other commentary in your answer. Structure your answer like this:
        ...
        if debug:
            print(f"\nSTORE MEMORY() LLM Response: {gpt_response}\n")

        # This is the MEMORY dictionary object
        memory_dict = json.loads(gpt_response)

        #Override description with precise_memory if set
        if precise_memory:
            memory_dict['memory_text'] = precise_memory
        ...
        if event_context_uuid is not None:
            ...
            if len(relevant_short_term_memories) > 0:
                ...
                short_term_memory_prompt = f"""
                    ...
                    If the first memory mentions the NAME of a person with whom you are speaking, assume you are STILL speaking with that person.
    ...
    def summarize_and_store_action_memory(self, action_summary='', model='gpt-4o-mini', debug=False):
        ...
        if action_summary == '':
            self._log("ERROR: Action Summary was empty. Cannot store memory.")
    ...
    def summarize_and_store_question_and_response_memory(self, question, response, llm_prompt='', model='gpt-4o', debug=False):
        ...
        #Find the activity name for this activity
        activity_uuid = find_activity(activity_description='Answering a verbal question', model = model, cursor=cursor, config=self.config, create_activity = True)
    ...
    def generate_backstory_memories(self, topic=None, do_random = False, age=None, num_memories=1, do_print=True,model='gpt-40-1'):
        ...
        self._log(f"INFO: CREATING: BACKSTORY MEMORIES FOR AGENT {self.uuid}")
    ...
    def make_next_decision(self,model='gpt-4o-mini',environment='',debug=False,event_context_uuid=None,action_result=None,exclude_long_term_memories=False,store_memory=True):
        # Set up variables
        current_time = datetime.utcnow()
        ...
        if action_result:
            #If an action_result was included in this method call, we need to store it to memory BEFORE we move foward...
            #NOTE: This is stored as a PRECISE memory in the Agent's memory stack. No interpretation / summarization
            action_result_dict = json.loads(action_result.replace("'", '"'))
            action_summary = f"ACTION TAKEN: {action_result_dict['action']}. RESULT OF ACTION: {action_result_dict['result']}"
            self.store_memory(activity_name = self.current_event['activity_name'], summary=action_summary, precise_memory=action_summary, debug=debug)
        ...
        llm_prompt = f"""Imagine that you are the PERSON described by the TRAITS below.
                ...
                Allude to your TRAITS in your RATIONALE, but reference your TRAITS only when VERY relevant.
        ...
        self.summarize_and_store_action_memory(action_summary=action_summary,debug=debug,model=model)
    ...
    def change_visibility(self, visibility=0):
        ...
        try:
            ...
            # Log changes
            self._log(f"INFO: UPDATED AGENT VISIBILITY. UUID: {self.uuid}")
    ...
    def create(self):
        ...
        # add self.client
        # TODO: Needs to be added to config.ini
        self.client = create_azure_client(config=self.config, model='gpt-40-1')
        ...
        #self._log(f"INFO: CREATION PROMPT: " + creation_prompt, _host=self.logging_host, _port=self.logging_port)
        self._log(f"======================")
    ...
    def shutdown(self):
        if self.memories_dict and hasattr(self.faiss_memories, 'reset'):
            self._log(f"INFO: UNLOADING AGENT FROM MEMORY {self.uuid}")
